...
const music_struct = {"introduction" : 0, "drones" : 2*60, "transe":4*60, "attention" : 5*60+20, "emotion" : 6*60+20, "cappella": 10*60, "final" : 12*60};
...
var DEMO_MODE = true;	// either test data or real EEG data
var is_sample_over = true;
var is_blinking = false;
let client_eeg;

function toySymphony(){
...
if(!DEMO_MODE){
	...
	// Node v18 https://github.com/node-fetch/node-fetch/issues/1624
	var options = {
	  host: "127.0.0.1",
	  appName: 'openminded',
	  appKey: '0fc2141b1b45c573cc2d3a763b8d71c5bde2391',
	  enableRawOutput: false, // false
	  format: 'Json' //'BinaryPacket'
	};


	client_eeg = thinkgear.createClient(options);
	client_eeg.connect();
...
function blink_bell(data, btSerial){
	...
	if("blinkStrength" in data  &&  data["blinkStrength"] >= THRESHOLD_BLINK && ! is_blinking){ 
...
function pulse(time, frequency) {
    pi = 3.14;
    noise = 0.01 *randomIntFromInterval(-1,1)
    //frequency = 0.12; // Frequency in Hz
    return 127*(1+ Math.sin(2 * pi * frequency * time)) + noise;
}


function blink_pulse(time, frequency) {
    pi = 3.14;
    //frequency = 0.12; // Frequency in Hz
    return 32.5*(1+ Math.sin(2 * pi * frequency * time));
} 


// Math.exp(-(time+timeSpent) / tau) * (intensity + 0.2*(Math.sin(6.28 * nu * time)))); 
// Math.exp(-(curr_time) / 1) * (0.8 + 0.2*(Math.sin(6.28 * 0.5 * curr_time)))); 
// {"delta":0, "theta":1, "lowAlpha":2, "highAlpha":3, "lowBeta":4, "highBeta":5, "lowGamma":6, "midGamma":7};
// 0.04, 0.07, 0.08, 0.13, 0.15, 0.2, 0.25, 0.4
function genDemoData(curr_time){
	...
	return {
		...
		"psd_scaled" : [pulse(curr_time,0.04),pulse(curr_time,0.07), pulse(curr_time,0.08)+pulse(curr_time,0.01),pulse(curr_time, 0.13), pulse(curr_time, 0.15),pulse(curr_time,0.2),pulse(curr_time,0.25),pulse(curr_time,0.4)],
		"blinkStrength" : Math.round(blink_pulse(curr_time,0.10)), // 255 pas 127
		"eSense": {
			"attention": Math.round(blink_pulse(curr_time,0.05)),//randomIntFromInterval(0,100),
			"meditation": Math.round(blink_pulse(curr_time,0.05))//randomIntFromInterval(0,100)
		}
...
function track_symphony(){
	...
	// demo mode
	if(DEMO_MODE){
		eeg_data = genDemoData(curr_time);
		blink_bell(eeg_data, btSerial);
	}

	eeg_symphony(curr_time, eeg_data, btSerial);
}
